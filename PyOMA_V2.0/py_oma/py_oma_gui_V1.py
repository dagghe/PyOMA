...
class UI(QMainWindow):
    ...
    def __init__(self):
        ...
        uic.loadUi(resolve_path("py_oma_V1.ui"), self)
    ...
    def clicker_load_data(self):
        ...
        if len(os.listdir(self.pathFolder)) != 0:
            ...
            if f_name:
                ...
                self.tableInputData.setRowCount(time_history)
                for i in range(time_history):
    ...
    def submit_peak(self):
        ...
        if num != self.no_channels:
            self.clear_peak()
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            msg.setWindowTitle("Identified Peaks Number Error")
            msg.setText(f'Please assign {self.no_channels} identified peaks')
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec_()
        else:
        ...
        and self.lineEditArrowScaleFactorFDD.text().isdigit() and int(self.lineEditArrowScaleFactorFDD.text()) > 0:
            ...
            for k in range(num_frames):
                ...
                if self.checkBoxDeformedShapeFDD.isChecked():
                    x1 = scale_factor_length_x * p1.xyz_new[0]
                    y1 = scale_factor_length_y * p1.xyz_new[1]
                    z1 = scale_factor_length_z * p1.xyz_new[2]
                    x2 = scale_factor_length_x * p2.xyz_new[0]
                    y2 = scale_factor_length_y * p2.xyz_new[1]
                    z2 = scale_factor_length_z * p2.xyz_new[2]
                    xx = [x1, x2]
                    yy = [y1, y2]
                    zz = [z1, z2]
            ...
            for i in range(nodes.shape[0]):
                ...
                xs2 = scale_factor_length_x * p1.xyz_new[0]
                ys2 = scale_factor_length_y * p1.xyz_new[1]
                zs2 = scale_factor_length_z * p1.xyz_new[2]
                if self.checkBoxDeformedShapeFDD.isChecked():
                    ax.scatter(xs2, ys2, zs2, color="red", marker='o')
                _x = round((xs2 - xs1)/scale_factor_length_x, 2)
                _y = round((ys2 - ys1)/scale_factor_length_y, 2)
                _z = round((zs2 - zs1)/scale_factor_length_z, 2)
    ...
    def write_to_gui(self, results, table_widget, table_name, name_method):
        ...
        if table_name == 'Frequencies':
            table_widget.setColumnCount(self.no_channels)
            for i in range(self.no_channels):
                _temp = f'f_{i + 1}'
                headers_labels.append(_temp)
            ...
            if name_method == 'FDD' or name_method == 'SSIcov' or name_method == 'SSIdat':
                for j in range(self.no_channels):
